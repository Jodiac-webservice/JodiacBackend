...
async function createShiprocketOrder(order, shippingAddress, orderItems, paymentMethod, totalAmount, retry = 0) {
    try {
        ...
        const shiprocketOrder = {
            order_id: order._id.toString(), // Shiprocket will generate its own ID
            channel_order_id: order._id.toString(), // Fix: Pass your MongoDB ID here
            order_date: new Date().toISOString().slice(0, 10),
            pickup_location: "threadora",
...
async function getShiprocketOrderDetails(shiprocketOrderId) {
    try {
        ...
        console.error("Shiprocket Get Order Details Error:", error.response?.data || error.message);
        throw error;
    }
}
